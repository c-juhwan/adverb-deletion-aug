...
def training(args: argparse.Namespace) -> None:
    ...
    if args.augmentation_type == 'none':
        dataset_dict['train'] = CustomDataset(args, os.path.join(args.preprocess_path, args.task, args.task_dataset, args.model_type, f'train_ORI.pkl'))
    elif args.augmentation_type in ['hard_eda', 'soft_eda', 'aeda', 'adverb_aug']:
        dataset_dict['train'] = CustomDataset(args, os.path.join(args.preprocess_path, args.task, args.task_dataset, args.model_type, f'train_{args.augmentation_type}.pkl'))
    elif args.augmentation_type == 'adverb_aug_curriculum':
        dataset_dict['train'] = CustomDataset(args, os.path.join(args.preprocess_path, args.task, args.task_dataset, args.model_type, f'train_ORI.pkl')) # First 2 epochs use original data
        dataset_dict['train_cur'] = CustomDataset(args, os.path.join(args.preprocess_path, args.task, args.task_dataset, args.model_type, f'train_{args.augmentation_type}.pkl')) # Next 3 epochs use augmented data
    dataset_dict['valid'] = CustomDataset(args, os.path.join(args.preprocess_path, args.task, args.task_dataset, args.model_type, f'valid_ORI.pkl'))
    ...
    for epoch_idx in range(start_epoch, args.num_epochs):
        ...
        if args.augmentation_type == 'adverb_aug_curriculum' and epoch_idx >= 2:
            # Change dataloader to augmented data after 2 epochs
            dataloader_dict['train'] = DataLoader(dataset_dict['train_cur'], batch_size=args.batch_size, num_workers=args.num_workers,
                                                  shuffle=True, pin_memory=True, drop_last=True)
