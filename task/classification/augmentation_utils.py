...
# You have to download the model if not downloaded yet
# python -m spacy download en
spacy_nlp = spacy.load('en_core_web_sm')

def run_adverb_aug(sentence: str, args: argparse.Namespace) -> str:
    """
    Main function to perform adverb deletion augmentation.
    Delete adverbs from the sentence, which are identified by SpaCy POS tagger.
    ...
    for token in parsed_sentence:
        if token.pos_ == 'ADV':
            augmented_sentence = augmented_sentence.replace(token.text, '')
            count += 1
    # Remove extra spaces
    augmented_sentence = re.sub(' +',' ',augmented_sentence)

    return augmented_sentence, count
